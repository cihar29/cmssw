...
void SiPixelPhase1Summary::fillTrendPlots(DQMStore::IBooker & iBooker, DQMStore::IGetter & iGetter, int lumiSec){
  ...
  for (unsigned int trendIt = 0; trendIt < trendOrder.size(); trendIt++){
	...
	if (tempLayerME->getBinContent(i,j) > 0.) nFilledROCs[trendIt]++;
          ...
          float x = tempLayerME->getTH1()->GetXaxis()->GetBinLowEdge(i);
          float y = tempLayerME->getTH1()->GetYaxis()->GetBinLowEdge(j);
          string key = trendNames[trendIt] + to_string(x) + to_string(y);

          if ( deadROCs_.find(key) == deadROCs_.end() ) {
            deadROCs_[key] = lumiSec;
            deadROCsFile_ << "Lumi " << lumiSec << "  " << trendNames[trendIt]
                          << "  ModuleCoord" << x << "  LadderCoord" << y << endl;
          }
        }
